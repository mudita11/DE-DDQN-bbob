...
class DEEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if self.budget <= 0:
            ...
            points[:, 0] /= np.log10(self.max_budget) # Normalize
            ...
            trace_file.close()
            shutil.rmtree("/local/data/data/ms1938/tf_env/batchsize64/DE-DDQN-bbob-copy/train-NN-R4-9/exdata/data/", ignore_errors = True)
            reward = cost
    ...
    def reset(self):
        ...
        self.lookup_file = "/local/data/data/ms1938/tf_env/batchsize64/DE-DDQN-bbob-copy/train-NN-R4-9/exdata/data/data_f"+str(func_file)
        ...
        self.F1 = np.zeros(self.NP) #np.apply_along_axis(self.fun, 1, self.u)
        self.budget -= self.NP
